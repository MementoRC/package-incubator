...
if is_linux; then
  ...
  # objcopy compiler-launcher: strips lld-unresolvable .deplibs records
  # (SHT_LLVM_DEPENDENT_LIBRARIES for pthread/rt) from libc++/libc++abi
  # object files that -fno-autolink above does not suppress (cxa_guard.cpp.o,
  # chrono.cpp.o -- see zig-strip-deplibs-launcher.sh for the full rationale
  # and the ZIG_STRIP_DEPLIBS=1 gate already bracketing both runtimes phases,
  # below and in _runtimes_target.sh). CMAKE_*_COMPILER_LAUNCHER applies to
  # compile rules only, never link rules -- exactly the granularity needed.
  # Appended to the shared _RUNTIMES_CMAKE array (not a phase-local copy) so
  # both the NATIVE clone (_NATIVE_RUNTIMES_CMAKE, cloned from this array
  # below) and _runtimes_target.sh's TARGET-arch cmake invocation (which
  # reuses this same array directly) pick it up -- both phases covered
  # without duplicating the flag.
  _strip_deplibs_launcher="${RECIPE_DIR}/building/zig-strip-deplibs-launcher.sh"
  if [[ ! -f "${_strip_deplibs_launcher}" ]]; then
    echo "ERROR: objcopy deplibs-strip launcher missing: ${_strip_deplibs_launcher}" >&2
    exit 1
  fi
  # git may not preserve the executable bit; chmod just-in-time so cmake/ninja
  # can invoke it directly as a compiler launcher (same pattern as the
  # macOS force-load shims in _zig_wrappers.sh).
  chmod +x "${_strip_deplibs_launcher}"
  _RUNTIMES_CMAKE+=(
    -DCMAKE_C_COMPILER_LAUNCHER="${_strip_deplibs_launcher}"
    -DCMAKE_CXX_COMPILER_LAUNCHER="${_strip_deplibs_launcher}"
  )
fi
...
if is_cross || is_osx; then
  ...
  if is_not_unix; then
    _native_cc="${BUILD_PREFIX}/Library/bin/${CONDA_ZIG_BUILD}-cc.exe"
    _native_cxx="${BUILD_PREFIX}/Library/bin/${CONDA_ZIG_BUILD}-cxx.exe"
  else
  ...
  # Windows: mirror _runtimes_target.sh's is_not_unix fix and the is_linux block
  # just above. Without an explicit -target here, zig-cxx auto-detects the GH
  # Actions runner's own VS2022 MSVC headers for this BUILD-arch (native) compile
  # instead of its bundled mingw headers, which manifests as libunwind's
  # DwarfParser.hpp _malloca/_freea undeclared errors (win-arm64 cross, native
  # libc++ phase). ZIG_TARGET_BUILD is already a gnu/mingw triple (recipe.yaml:55,
  # "x86_64-windows-gnu" for build_platform == win-64) -- no glibc-style suffix
  # needed here, unlike the is_linux ".2.17" floor above.
  if is_not_unix; then
    _NATIVE_RUNTIMES_CMAKE+=(
      -DCMAKE_C_COMPILER_TARGET="${ZIG_TARGET_BUILD}"
      -DCMAKE_CXX_COMPILER_TARGET="${ZIG_TARGET_BUILD}"
      -DCMAKE_ASM_COMPILER_TARGET="${ZIG_TARGET_BUILD}"
      "-DCMAKE_ASM_FLAGS=--target=${ZIG_TARGET_BUILD}"
    )
  fi
  ...
  # WORKAROUND: hide MACOSX_DEPLOYMENT_TARGET from the NATIVE (build-arch) build.
  # The zig-cc wrapper used here comes from a PREBUILT zig package resolved from
  # conda-forge/prefix.dev -- NOT from this repo's recipes/zig-0.16.0 build, which
  # runs AFTER zig-llvm-16 in every lane (confirmed via the resolve table in PR #17
  # run 31289796068 job 93186540809: zig_osx-64/zig_osx-arm64 came from conda-forge).
  # That wrapper's triple substitution assumes an ABI suffix which the BUILD-arch
  # triple lacks, so when MACOSX_DEPLOYMENT_TARGET is set it emits
  # 'aarch64-macos.11.3-aarch64-macos.11.0' and zig fails with
  # "unable to parse target query ...: UnknownApplicationBinaryInterface".
  # The wrapper already bakes the correct build-arch deployment version, so simply
  # hiding the variable for this build is sufficient. Fixed at source in
  # recipes/zig-0.16.0/scripts/_zig-cc-common.sh, but that fix cannot reach this
  # build until a zig package with a higher build number is published.
  #
  # Use save/restore, NOT a subshell: the failure paths below call `exit`, and a
  # subshell would swallow those and let the build continue after a FATAL.
  _mdt_was_set=0
  if [[ -n "${MACOSX_DEPLOYMENT_TARGET+x}" ]]; then
    _mdt_was_set=1
    _mdt_saved="${MACOSX_DEPLOYMENT_TARGET}"
    unset MACOSX_DEPLOYMENT_TARGET
    echo "  native build-arch: temporarily unset MACOSX_DEPLOYMENT_TARGET (was ${_mdt_saved}) to avoid prebuilt zig-cc triple mangling"
  fi
  ...
  if (( _mdt_was_set )); then
    export MACOSX_DEPLOYMENT_TARGET="${_mdt_saved}"
    unset _mdt_saved
    echo "  native build-arch: restored MACOSX_DEPLOYMENT_TARGET=${MACOSX_DEPLOYMENT_TARGET}"
  fi
  unset _mdt_was_set
