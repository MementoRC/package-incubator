function remove_unneeded() {
  if [[ -f "${SRC_DIR}/.zig_local_iterate" ]]; then
    ...
    if [[ "${target_platform}" == "linux-riscv64" || "${target_platform}" == "linux-s390x" || "${target_platform}" == "linux-ppc64le" ]]; then
      # linux-riscv64/linux-s390x: no liblldZig.so bundle is built for these arches
      # (no conda-forge zstd/xml2/z available), so keep the six liblld*.a archives
      # permanently for zig-zig to link statically.
      # linux-ppc64le: liblldZig.so IS built, but zig's own build (zig-0.16.0
      # recipe, building/_lld_bundle.sh) relinks the six archives with -mlongcall
      # into its own R_PPC64_REL24-safe bundle, consuming them from this package
      # as an installed build dependency. Keep them for that consumer.
      find "${LLVM_INSTALL}/lib" -name "*.a" ! -name "*.dll.a" ! -name "liblld*.a" -type f -delete
      echo "  Removed non-lld .a files from ${LLVM_INSTALL}/lib (kept .dll.a import libs; kept liblld*.a for ${target_platform})"
    else
      # Everywhere else (linux-64/aarch64, osx-64/arm64, win-*): zig links against
      # the already-built liblldZig.so/.dylib/.dll bundle (ZIG_LLD_BUNDLE_SO in
      # zig-0.16.0/building/build-zig.sh) instead of the raw archives, so drop them
      # now (kept .dll.a import libs on Windows).
