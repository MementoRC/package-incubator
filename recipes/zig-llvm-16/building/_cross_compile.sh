...
NATIVE_CMINIT
  ...
  # Also force this for ALL windows targets (is_not_unix), including win-64
  # NATIVE (build_platform == target_platform, so is_cross is false there):
  # without CMAKE_C_ABI_COMPILED=TRUE/CMAKE_CXX_ABI_COMPILED=TRUE, cmake's
  # compiler-ABI-detection probe builds a default test EXE, which on Windows
  # zig-cc/lld-link injects a bare "-Xlinker /version:0.0" into — zig's linker
  # rejects that 2-component version string as InvalidVersion, so the probe
  # (CMakeTestCCompiler.cmake) fails outright ("Check for working C compiler:
  # ...zig-cc.exe - broken"), aborting the whole LLVM cmake configure before
  # build.ninja is even generated. Setting *_ABI_COMPILED=TRUE skips that probe
  # entirely. Mirrors the identical fix already applied to the runtimes-build
  # cmake configure in _runtimes_target.sh for the same win-64-native gap.
...
# Native Windows (build_platform == target_platform) never enters the
# CONDA_BUILD_CROSS_COMPILATION guard above, so the CMAKE_C/CXX_ABI_COMPILED
# workaround at line ~213 (which skips CMake's CMakeTestCCompiler probe) never
# fires for it. On win-64 native, that probe builds a default test EXE which
# zig lld-link rejects via a bare "-Xlinker /version:0.0" (InvalidVersion),
# aborting the zig-libcxx LLVM cmake configure before build.ninja is even
# generated (PR #17 CI regression). Mirror the top-level guard already used in
# _runtimes_target.sh (is_cross || CONDA_BUILD_CROSS_COMPILATION==1 ||
# is_not_unix) by adding an equivalent block here for the case the guard above
# doesn't cover: native (non-cross) Windows. ZIG_LLVM_TRIPLET is recomputed
# independently (same precedent as _runtimes_build.sh:249) since the copy at
# line 194 only runs inside the CONDA_BUILD_CROSS_COMPILATION guard.
if [[ "${CONDA_BUILD_CROSS_COMPILATION:-0}" != "1" ]] && is_not_unix; then
  ZIG_LLVM_TRIPLET="$(zig_triplet_from_llvm "${LLVM_TRIPLET}")"
  CMAKE_CROSS_FLAGS+=(
    -DCMAKE_C_COMPILER_TARGET="${ZIG_LLVM_TRIPLET}"
    -DCMAKE_CXX_COMPILER_TARGET="${ZIG_LLVM_TRIPLET}"
