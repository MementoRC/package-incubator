...
# Also force this for ALL windows targets (is_not_unix), including win-64 NATIVE
# (build_platform == target_platform, so is_cross is false there): without an
# explicit -target, zig-cxx falls back to auto-detecting the GH Actions windows
# runner's own VS2022 MSVC toolchain instead of using its bundled mingw headers,
# which manifests as libunwind/libcxxabi compile failures (_malloca/_freea
# undeclared in DwarfParser.hpp; type_info redeclaration colliding with MSVC's
# vcruntime_typeinfo.h) — the exact same symptom already fixed for win-arm64's
# liblldZig.dll build (see _lld_bundle.sh, an is_cross=true cross build) by
# passing an explicit -target there.
if is_cross || [[ "${CONDA_BUILD_CROSS_COMPILATION:-0}" == "1" ]] || is_not_unix; then
  _RUNTIMES_CMAKE+=(
...
# _RUNTIMES_CMAKE already carries -DCMAKE_C_COMPILER_LAUNCHER/-DCMAKE_CXX_
# COMPILER_LAUNCHER=zig-strip-deplibs-launcher.sh on Linux (appended in
# _runtimes_build.sh's is_linux block). Nothing between there and here
# (build_native_llvm_config / _native_llvm_config.sh) mutates this array, so
# this TARGET-arch cmake invocation below inherits it unchanged -- no
# duplicate -D flag needed. This covers the TARGET phase; the NATIVE
# build-arch phase is covered separately via _NATIVE_RUNTIMES_CMAKE (cloned
# from this same array in _runtimes_build.sh).
