...
build_native_llvm_config() {
  ...
  # macOS equivalent of the is_linux override above. Unlike linux, this is NOT a
  # CMAKE_*_COMPILER_TARGET override: zig_build_triplet's osx branches (recipe.yaml)
  # already emit a fully-versioned single triple (e.g. "aarch64-macos.11.0"), so
  # appending ".2.17" the way the linux branch does would itself double the version
  # suffix. The actual doubling here has a different root cause: this build-arch
  # ${CONDA_ZIG_BUILD}-cc/-cxx wrapper is the SAME prebuilt (conda-forge/prefix.dev)
  # zig-cc wrapper used by _runtimes_build.sh's native libc++ step, and its triple
  # substitution mangles the BUILD-arch triple whenever MACOSX_DEPLOYMENT_TARGET is
  # set, emitting e.g. 'aarch64-macos.11.3-aarch64-macos.11.0':
  # UnknownApplicationBinaryInterface (log-verified CI run 31321865432, osx-64
  # cross) -- identical to the failure mode already diagnosed and fixed for that
  # sibling native build (_runtimes_build.sh WORKAROUND, ~line 405-427). The wrapper
  # already bakes the correct build-arch deployment version itself, so hiding the
  # env var for this call is sufficient; no -target override is needed for
  # CC/CXX/ASM (all three route through this single env var here, unlike the
  # per-compiler CMAKE_*_COMPILER_TARGET array the linux branch needs).
  local _mdt_saved=""
  local _mdt_was_set=0
  if is_osx && [[ -n "${MACOSX_DEPLOYMENT_TARGET+x}" ]]; then
    _mdt_was_set=1
    _mdt_saved="${MACOSX_DEPLOYMENT_TARGET}"
    unset MACOSX_DEPLOYMENT_TARGET
    echo "  native build-arch llvm-config: temporarily unset MACOSX_DEPLOYMENT_TARGET (was ${_mdt_saved}) to avoid prebuilt zig-cc triple mangling"
  fi
  ...
  if (( _mdt_was_set )); then
    ...
    echo "  native build-arch llvm-config: restored MACOSX_DEPLOYMENT_TARGET=${MACOSX_DEPLOYMENT_TARGET}"
  fi
