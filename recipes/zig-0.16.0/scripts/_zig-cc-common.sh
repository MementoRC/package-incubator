...
#
# Two triple FORMATS reach this wrapper and they differ in whether an ABI
# suffix is present (recipe.yaml): the TARGET triple carries one
# ("aarch64-macos.11.3-none") while the BUILD triple does NOT
# ("aarch64-macos.11.0"). The previous one-liner used
# ${_zig_target##*macos*-}, which needs a '-' AFTER "macos"; on the bare
# build-arch form that glob never matches, so ## returned the WHOLE original
# string and it got self-concatenated onto the rebuilt triple, producing
# 'aarch64-macos.11.3-aarch64-macos.11.0' and zig's
# "unable to parse target query ...: UnknownApplicationBinaryInterface"
# (PR #17 run 31281895675, osx-64 cross, native build-arch libc++).
# Split explicitly and only re-append an ABI segment when one actually exists.
_zig_target="@ZIG_TARGET@"
if [[ -n "${MACOSX_DEPLOYMENT_TARGET:-}" ]] && [[ "${_zig_target}" == *-macos* ]]; then
    _zt_arch="${_zig_target%%-macos*}"
    _zt_rest="${_zig_target#*-macos}"
    _zt_abi=""
    if [[ "${_zt_rest}" == *-* ]]; then
        _zt_abi="-${_zt_rest#*-}"
    fi
    _zig_target="${_zt_arch}-macos.${MACOSX_DEPLOYMENT_TARGET}${_zt_abi}"
    unset _zt_arch _zt_rest _zt_abi
fi
