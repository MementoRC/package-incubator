...
function configure_cmake() {
  ...
  # Build local cmake args array. No compiler is pinned here by default (CMake
  # auto-detects the ambient CC/CXX); callers that need a specific compiler (e.g.
  # build-zig.sh pinning zigcpp's configure to the zig-cc stub wrappers on linux,
  # for libstdc++/libc++ ABI consistency with zig-llvm) pass -DCMAKE_C_COMPILER/
  # -DCMAKE_CXX_COMPILER via the global EXTRA_CMAKE_ARGS merged in below.
  local cmake_args=()
