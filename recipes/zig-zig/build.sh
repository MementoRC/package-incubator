...
if is_linux && is_cross; then
  ...
  create_pthread_atfork_stub "${CONDA_TRIPLET%%-*}" "${CC}" "${ZIG_LOCAL_CACHE_DIR}" "${ZIG_TRIPLET}"
  ...
  create_libc_single_threaded_stub "${CONDA_TRIPLET%%-*}" "${CC}" "${ZIG_LOCAL_CACHE_DIR}" "${ZIG_TRIPLET}"
fi
