...
# Inline debug helper (mirrors zig-llvm _env.sh pattern, but uses ZIG_ZIG_DEBUG).
# This file is sourced by build.sh which does not pre-define dbg, so we define it here.
_debug() { [[ "${ZIG_ZIG_DEBUG:-0}" == "1" ]]; }
