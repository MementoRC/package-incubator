...
# Discover the triple prefix used for wrapper filenames (e.g. x86_64-linux-gnu)
# by globbing for *-zig-cc or *-zig-cc.exe in the wrapper directory.
import glob as _glob
_zig_prefix_candidates = _glob.glob(str(_wrapper_dir / ("*-zig-cc.exe" if _build_is_win else "*-zig-cc")))
_zig_wrapper_prefix: str = ""
if _zig_prefix_candidates:
    _stem = Path(_zig_prefix_candidates[0]).name
    # stem is e.g. "x86_64-linux-gnu-zig-cc" or "x86_64-linux-gnu-zig-cc.exe"
    _stem = _stem.replace(".exe", "")
    # remove "-zig-cc" suffix
    if _stem.endswith("-zig-cc"):
        _zig_wrapper_prefix = _stem[: -len("-zig-cc")]
...
def test_wrapper_existence() -> None:
    ...
    pfx = _zig_wrapper_prefix + "-" if _zig_wrapper_prefix else ""
    ...
    if _build_is_win:
        ...
        expected = [
            f"{pfx}zig-cc",
            f"{pfx}zig-cxx",
            f"{pfx}zig-force-load-cc",
            f"{pfx}zig-force-load-cxx",
            f"{pfx}zig-ar",
            f"{pfx}zig-ranlib",
            f"{pfx}zig-asm",
            f"{pfx}zig-rc",
...
def test_flag_filtering() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if _is_emulated or _is_cross_compiler:
            SKIP("compile with conda gcc flags", "emulated/cross CI — zig may need GCC internally for this target")
        else:
            gcc_flags = [
                "-march=nocona",
                "-mtune=haswell",
...
def _test_shared_lib_windows(zig_cc: str, obj: Path, td: str) -> None:
    ...
    if not zig_ar:
        # Fallback: try wrapper dir with triple prefix
        pfx = _zig_wrapper_prefix + "-" if _zig_wrapper_prefix else ""
        candidate = _wrapper_dir / (f"{pfx}zig-ar.bat" if _build_is_win else f"{pfx}zig-ar")
        if candidate.exists():
...
def test_windows_import_libs() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if r2.stderr == "TIMEOUT":
            ...
            elif "unable to automatically import" in r2.stderr and "IMAGE_REL_ARM64" in r2.stderr:
                # lld-link cannot auto-import _fpreset via ARM64 branch relocations
                # (IMAGE_REL_ARM64_BRANCH26) in MinGW CRT objects. This is an lld
                # limitation on ARM64, not a feedstock bug — no unreachable panic.
                WARN(
                    "windows import libs (-lapi-ms-win-core-synch-l1-2-0)",
                    "lld-link ARM64 auto-import limitation (CRT _fpreset) — no unreachable panic, fix OK",
                )
            else:
                FAIL(
                    "windows import libs (-lapi-ms-win-core-synch-l1-2-0)",
...
def test_force_load_wrappers() -> None:
    ...
    pfx = _zig_wrapper_prefix + "-" if _zig_wrapper_prefix else ""
    fl_cc = _wrapper_dir / f"{pfx}zig-force-load-cc"
    if not fl_cc.exists():
        FAIL(f"{pfx}zig-force-load-cc exists")
        return
    ...
    fl_cxx = _wrapper_dir / f"{pfx}zig-force-load-cxx"
    if not fl_cxx.exists():
        FAIL(f"{pfx}zig-force-load-cxx exists")
        return
