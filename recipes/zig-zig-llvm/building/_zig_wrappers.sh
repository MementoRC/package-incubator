# Use the zig C-wrapper scripts provided by the self-hosted zig-zig package.
# Unlike the upstream zig_impl package (which installs its dispatchers into bin/),
# zig-zig ships its triple-prefixed wrapper scripts under share/zig/wrappers/.
# Layout:
#   Unix:    $BUILD_PREFIX/share/zig/wrappers/${CONDA_BUILD_ZIG}-{cc,cxx,ar,ranlib,asm,rc,force-load-cc,force-load-cxx}
...
if is_not_unix; then
  ...
  # zig-zig installs its triple-prefixed wrapper scripts under
  # share/zig/wrappers/ (NOT bin/, unlike the upstream zig_impl package that
  # zig-llvm consumes). Prefer that location; fall back to bin/ if a bin/-based
  # layout is present. (zig-llvm's own _zig_wrappers.sh keeps using bin/.)
  _ext=""
  _zig_bindir="${BUILD_PREFIX}/share/zig/wrappers"
  [[ -x "${_zig_bindir}/${CONDA_BUILD_ZIG}-cc" ]] || _zig_bindir="${BUILD_PREFIX}/bin"
fi

_probe_cc="${_zig_bindir}/${CONDA_BUILD_ZIG}-cc${_ext}"
if [[ ! -x "${_probe_cc}" ]]; then
  echo "ERROR: zig cc wrapper not found at ${_probe_cc}"
  echo "  Expected zig-zig_${build_platform} to install ${CONDA_BUILD_ZIG}-cc under share/zig/wrappers/ (or bin/)."
