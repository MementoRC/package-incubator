...
# --- Derive zig target from the invoked wrapper's filename ---
# _zig-cc-common.sh is installed once per PREFIX but all per-target wrappers
# (aarch64-linux-gnu-zig-cc, riscv64-linux-gnu-zig-cc, …) source it.
# Callers set _zig_wrapper_invoked="$BASH_SOURCE[0]" before sourcing so we can
# recover the correct target from the wrapper filename at runtime.
_zig_target="@ZIG_TARGET@"
_zig_target_arch="@ZIG_TARGET_ARCH@"
if [[ -n "${_zig_wrapper_invoked:-}" ]]; then
    _wrapper_name="$(basename "${_zig_wrapper_invoked}")"
    case "${_wrapper_name}" in
        *-zig-cc|*-zig-cxx|*-zig-c++|*-zig-asm|*-zig-ar|*-zig-ranlib|*-zig-rc|*-zig-lld|*-zig-force-load-cc|*-zig-force-load-cxx)
            _zig_target="${_wrapper_name%-zig-*}"
            _zig_target_arch="${_zig_target%%-*}"
            ;;
    esac
    unset _wrapper_name
fi
...
while [[ $i -lt $argc ]]; do
    ...
    case "$arg" in
            ...
            if [[ $next_i -lt $argc ]]; then
                ...
                case "$next_arg" in
                    --color-diagnostics|--dependency-file=*)
                        i=$next_i ;;
                    # Filter bare -l and known-bad lib refs when wrapped in -Xlinker.
                    # cmake can emit `-Xlinker -l` when a library variable expands
                    # empty (e.g. PTHREAD_LIB on macOS). Bare -l reaches ld64.lld
        ...
        # macOS: cmake generates bare -l (empty library name) for libraries that
        # live in libSystem (pthread, dl, atomic) — those vars are empty on macOS.
        # ld64.lld rejects these with "library not found for -l"; filter them.
        # Match exact "-l" plus whitespace-padded variants ("-l ", "-l\t") that
        # can arise from quoted cmake var expansion.
        # Two-arg form: cmake emits -l "${EMPTY_VAR}" → argv sees "-l" then "".
        # Consume the empty next-arg so it never reaches the *)  branch.
        -l)
            next_i=$((i + 1))
            if [[ $next_i -lt $argc && -z "${argv[$next_i]}" ]]; then
                i=$next_i  # consume the empty arg that follows bare -l
            fi
            ;;
        -Wl,-syslibroot|-Wl,-syslibroot,*)
            # zig's native macOS linker handles SDK via -isysroot; -Wl,-syslibroot
            # is unsupported. Drop it for macOS targets where we don't use external lld.
            case "${_zig_target}" in
                *-macos*|*-darwin*) ;;  # drop on macOS
        ...
        *)
            ...
            if [[ -z "$arg" ]]; then
                :
            elif [[ "$arg" =~ ^-l[[:space:]]+$ ]]; then
                : # bare -l with trailing whitespace — drop
            else
                args+=("$arg")
            fi
            ;;
...
# (_zig_target was derived from wrapper filename or @ZIG_TARGET@ fallback at top of file)
if [[ -n "${MACOSX_DEPLOYMENT_TARGET:-}" ]] && [[ "${_zig_target}" == *-macos* ]]; then
    _arch_part="${_zig_target%%-macos*}"
    _post_macos="${_zig_target#*-macos.}"
    if [[ "${_post_macos}" == *-* ]]; then
        _abi_suffix="-${_post_macos#*-}"
    else
        _abi_suffix=""
    fi
    _zig_target="${_arch_part}-macos.${MACOSX_DEPLOYMENT_TARGET}${_abi_suffix}"
fi
...
if [[ -z "${_macos_syslibroot:-}" ]] && [[ "@ZIG_TARGET@" == *-macos* ]]; then
    _auto_sdk=""
    for _cand in \
        "${SDKROOT:-}" \
        "${CONDA_BUILD_SYSROOT:-}" \
