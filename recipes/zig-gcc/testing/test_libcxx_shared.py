...
# Cross-compilation detection: target arch != build machine arch
_is_cross_compiled = (
    (is_arm64 and _native_machine not in ("aarch64", "arm64"))
    or (is_ppc64le and _native_machine != "ppc64le")
)
...
def test_libcxx_fallback_static() -> None:
    ...
    if _is_cross_compiled or _is_emulated:
        SKIP("libcxx-static-fallback", "cross-compiled/emulated, skip linking tests")
        return
...
def test_libcxx_probe_paths() -> None:
    ...
    if _is_cross_compiled or _is_emulated:
        SKIP("libcxx-probe", "cross-compiled/emulated, skip linking tests")
        return
...
def test_libcxx_shared_simulation() -> None:
    ...
    if _is_cross_compiled or _is_emulated:
        SKIP("libcxx-simulation", "cross-compiled/emulated, skip linking tests")
        return
...
def test_libcxx_env_override() -> None:
    ...
    if _is_cross_compiled or _is_emulated:
        SKIP("libcxx-env-override", "cross-compiled/emulated, skip linking tests")
        return
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # On cross-compiled targets, zig c++ builds the entire libc++ from
        # source -- too expensive under emulation (OOM kill).
        if _is_cross_compiled:
            PASS("libcxx-env-override: patch compiled in + shared lib found "
                 "(compile test deferred to native build)")
            return
