...
def test_windows_import_libs() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if r2.stderr == "TIMEOUT":
            ...
            elif is_aarch64_win and "IMAGE_REL_ARM64_BRANCH26" in r2.stderr:
                # ARM64 MinGW limitation: lld-link cannot auto-import via
                # BRANCH26 relocations (32-bit displacement). Not a feedstock
                # bug — the unreachable panic (what we fixed) is absent.
                WARN(
                    "windows import libs (-lapi-ms-win-core-synch-l1-2-0)",
                    "IMAGE_REL_ARM64_BRANCH26 auto-import — ARM64 MinGW limitation, not feedstock bug",
                )
            else:
                FAIL(
                    "windows import libs (-lapi-ms-win-core-synch-l1-2-0)",
