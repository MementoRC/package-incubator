...
def main():
    ...
    wrapper_triplet = os.environ.get("ZIG_WRAPPER_TRIPLET", zig_triplet)
    ...
    if cross_compiler == "true":
        ...
        if is_nonunix:
            install_nonunix_cross_wrappers(prefix, recipe_dir, native_triplet, target_triplet, wrapper_triplet)
        else:
            install_unix_cross_wrappers(prefix, recipe_dir, native_triplet, target_triplet, wrapper_triplet)
...
def install_activation_scripts(
    ...
    replacements = {
        ...
        "@ZIG_TARGET@": _strip_glibc_version(zig_triplet),
    }
