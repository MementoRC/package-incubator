...
if [[ "${target_platform}" == "linux-ppc64le" ]]; then
  ...
  # Non-fatal diagnostic: confirm whether zig-llvm's libunwind files actually
  # landed on disk before the symlink guard below runs. Never allowed to fail
  # the build.
  echo "=== ppc64le libunwind diagnostic ===" || true
  ls -la "${PREFIX}/lib/zig-llvm/lib/" 2>&1 || echo "  DIRECTORY MISSING"
  ls -la "${PREFIX}/lib/zig-llvm/lib/" 2>/dev/null | grep -i libunwind || echo "  no libunwind entries found in listing"
  if [[ -f "${PREFIX}/lib/zig-llvm/lib/libunwind.so.1.0" ]]; then
    echo "  FOUND: ${PREFIX}/lib/zig-llvm/lib/libunwind.so.1.0 present" || true
  else
    echo "  MISSING: ${PREFIX}/lib/zig-llvm/lib/libunwind.so.1.0 not present" || true
  fi
  if [[ -e "${PREFIX}/lib/zig-llvm/lib/libunwind.so" ]]; then
    echo "  FOUND: ${PREFIX}/lib/zig-llvm/lib/libunwind.so already exists" || true
  else
    echo "  MISSING: ${PREFIX}/lib/zig-llvm/lib/libunwind.so not present" || true
  fi
...
if is_osx; then
  ...
  # riscv64 EXCLUDED from ZIG_SYSTEM_LIBCXX=stdc++ (the general linux/windows
  # value below): unlike ppc64le (has gcc_linux-ppc64le/binutils_linux-ppc64le
  # build deps, build.sh:276-281) riscv64 has no riscv64 cross-gcc install, so
  # zig-cc's Clang-style GCC-installation auto-detection for "stdc++" falls
  # back to the BUILD host's native x86_64 GCC (/usr/lib/gcc/x86_64-linux-gnu/
  # 14/libstdc++.so), which ld.lld then rejects at the final zig link with
  # "incompatible with elf64lriscv" (CI run 31066313273). "stdc++" is also a
  # GCC-toolchain artifact riscv64 never has locally, whereas "c++" (LLVM's
  # libc++, matching what zig-llvm actually ships and what osx uses above)
  # isn't something Clang's GCC-detection fallback resolves at all, and is
  # wired explicitly from zig-llvm's bundled copy below (build.sh:~586).
...
if is_linux && is_cross; then
  ...
  #
  # riscv64 EXCLUDED: recipe.yaml's build requirements skip stdlib('c') entirely
  # for riscv64 (`if: not riscv64`), so no sysroot_linux-riscv64 package is ever
  # installed and this fallback path (${BUILD_PREFIX}/${CONDA_TRIPLET}/sysroot)
  # does not contain a real target libc (no wchar.h etc). Feeding that bogus path
  # to zig via --libc/--libc-runtimes makes the self-hosted riscv64 compile
  # resolve <string_view>'s wchar.h-dependent bits (ItaniumDemangle.h /
  # StringViewExtras.h in libc++abi's cxa_demangle.cpp) against an incomplete
  # sysroot, producing "mbstate_t"/"wcschr"/"EOF" undeclared errors (CI run
  # 31046688653). zig-zig/build.sh already excludes riscv64 from this exact
  # wiring (is_riscv64 guards at build.sh:182,456,759) and relies on zig's own
  # bundled riscv64-linux-gnu.2.39 libc metadata instead; mirror that here.
...
# riscv64: excluded from the native-link block above because its bare
# -lzstd/-lxml2/-lz and liblldZig.so additions are wrong for riscv64 (it uses
# namespaced zig-zstd/zig-xml2/zig-zlib packages, build.sh:560-565, and a
# separate liblld*.a static-archive wiring, build.sh:358-361). It still needs
# the C++ runtime wired explicitly though, since ZIG_SYSTEM_LIBCXX=c++ above
# (build.sh:~239) only steers CMake's link-library *name* for zigcpp, not the
# final self-hosted zig link. Pull zig-llvm's bundled libc++/libc++abi/
# libunwind directly, same as the native-link arches (build.sh:578-580).
if [[ "${target_platform}" == "linux-riscv64" ]]; then
  _lld_lib="${PREFIX}/lib/zig-llvm/lib"
  perl -pi -e "s@(ZIG_LLVM_LIBRARIES \".*)\"@\$1;-L${_lld_lib};-lc++;-lc++abi;-lunwind\"@" "${cmake_build_dir}"/config.h
fi
...
if is_linux && is_cross; then
  ...
  # riscv64 EXCLUDED: no CONDA_BUILD_SYSROOT-backed libc is wired for riscv64
  # (see the is_linux && is_cross block above); creating this file would embed
  # ${BUILD_PREFIX}/${CONDA_TRIPLET}/sysroot/usr/include as include_dir/
  # sys_include_dir even though that directory doesn't exist for riscv64.
  if [[ "${target_platform}" != "linux-riscv64" ]]; then
  ...
  # zig-cc-early has no baked-in target (build.sh:394); without --target= it
  # compiles the stub for the build platform, causing an ELF class mismatch
  # at the final self-hosted link (e.g. "incompatible with elf64-littleaarch64"
  # on linux-aarch64). Pass --target=${ZIG_TRIPLET}, same precedent as the
  # CMAKE_C_FLAGS/CMAKE_CXX_FLAGS target flags set above (build.sh:400-401).
  create_libc_single_threaded_stub "${SRC_DIR}/zig-cc-early" "${ZIG_LOCAL_CACHE_DIR}" "--target=${ZIG_TRIPLET}"
elif is_linux; then
...
#   a bare `-idirafter` was observed to be ineffective for this triple
#   (x86_64-macos.11.0-none) — LibCDirs' triple-based auto-detection still
#   fell back to zig's bundled freestanding headers. NOTE: that attempt
#   predated the array-vs-scalar fix documented below (_WRAPPER_CC_EXTRA was
#   a plain string at the time); it is plausible -idirafter was actually
#   delivered to `zig cc` as one malformed, space-joined argv token rather
#   than two separate tokens, which would also explain the symptom. That
#   confound was never isolated. The follow-up "fix" (`--libc <path>`,
#   zig's libc.txt-file mechanism) was WRONG: `--libc` is only recognized by
#   zig's native build-exe/build-obj/run/test arg parser (src/main.zig,
#   .build/.run/.zig_test branch) and is ABSENT from the Clang-compatibility
#   option table (src/clang_options_data.zig) consulted for `zig cc`/`zig
#   c++`, so it hard-fails with "Unknown Clang option: '--libc'" (verified
#   by reading zig 0.15.2 src/main.zig's ClangArgIterator dispatch, run
#   31046688653). `--sysroot` IS in that table, but only sets link-time
#   sysroot metadata (Compilation.zig: `options.sysroot`) — it is NOT fed
#   into `LibCDirs.detect()` and does not influence the header search list,
#   so it would not have fixed the stdio.h lookup either.
#   Current fix: retry the header-injection approach, now via a properly
#   quoted bash ARRAY element (no space-joining risk), passing BOTH
#   `-idirafter` and `-isystem` for the same directory. `-isystem` mirrors
#   exactly the mechanism zig's own LibCDirs auto-detection uses internally
#   for equivalent already-working targets (Compilation.zig ~line 6991-98:
#   each libc_include_dir_list entry is emitted as `-isystem <dir>`), so it
#   should be honored even if auto-detection itself fails to populate that
#   list for this triple. `-idirafter` is kept alongside it as a low-priority
#   fallback. CONFIDENCE: MEDIUM — the previous `-idirafter`-dropped result
#   was never re-tested after switching to array-safe quoting, so this may
#   simply have been the array/scalar quoting bug all along; needs CI
#   verification (not yet re-run at time of writing).
_WRAPPER_OSX_LIBC_DIR="${PREFIX}/lib/zig/libc/include/any-macos-any"
...
case "${target_platform}" in
    ...
    osx-*)
        if is_cross; then
          _WRAPPER_CC_EXTRA=(--target="${ZIG_TRIPLET}" -idirafter "${_WRAPPER_OSX_LIBC_DIR}" -isystem "${_WRAPPER_OSX_LIBC_DIR}")
        fi
        ;;
esac
...
# Cross builds: ${PREFIX}/bin/zig is the just-built TARGET-arch binary (e.g. an
# aarch64 ELF on an x86_64 linux-64 runner) and cannot execute natively on the
# build host, so it must be wrapped under qemu-user, same as the Phase 2
# langref invocation above (_stage3_runner). Reconstructed locally here
# (rather than reusing _stage3_runner) since that array is only set inside
# the conditional Phase 2 block and may be unset (SKIP_LANGREF=1, or
# non-linux cross) by the time this line runs, and `set -u` is active.
PRIMARY_WRAPPER="${WRAPPER_BIN_DIR}/${CONDA_TRIPLET}-zig${EXE_EXT}"
_wrapper_zig_runner=()
if is_cross && is_linux; then
  _wrapper_zig_runner=("qemu-${ZIG_QEMU_ARCH}")
fi
"${_wrapper_zig_runner[@]+"${_wrapper_zig_runner[@]}"}" "${PREFIX}/bin/zig" cc -O2 "${_WRAPPER_CC_EXTRA[@]}" ${WRAPPER_LDFLAGS} -iquote "${RECIPE_DIR}/building" "${WRAPPER_C}" -o "${PRIMARY_WRAPPER}"
